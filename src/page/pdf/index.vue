  ...
  <div class="pdf_container">
    ...
    <div class="pdf_content">
      ...
      <table class="pdf_info" border="1">
        ...
        <tr>
          <td>价格{{allTotalPrice.price_unit}}</td>
        ...
        <tr>
          <td style="text-align: left;" colspan="3">物料:&nbsp;&nbsp;&nbsp;{{Text}}</td>
        </tr>
        <tr>
...
export default {
  ...
  computed: {
    ...mapState(["formInfo", "orderInfo", "allTotalPrice"])
  },
  mounted() {
    this.priceList();
  },
      ...
      if (orderInfo.offerNumber) {
        axios({
          ...
          if (res.data.rows !== null) {
            this.price_list = JSON.parse(res.data.rows);
            ...
            // );
            let array1 = this.price_list.bom;
            for (let i = 0; i < array1.length; i++) {
              this.Text += array1[i].parts_name + " : " + array1[i].itemname;
            }
            let array2 = this.price_list.glue;
            array2.forEach(item => {
              this.glue += item.parts_name + " : " + item.glueName1;
            });
          }
        });
      }
